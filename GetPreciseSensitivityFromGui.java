...
public class GetPreciseSensitivityFromGui {
    ...
    public static void main(String[] args) {
        ...
        int framebufferheight = 2586;
        int framebufferwidth = 1690;
        double clickCoord = 98.72389791183295;

        Screen screen = new Screen(framebufferheight, framebufferwidth, false);

        int boxStart = (screen.scaledWidth / 2) - 155;
        double clickRawCoord = Math.round((clickCoord/screen.scaledWidth)*screen.width);
        // double relativeClickSpot = Math.round(clickCoord/)
        double boxPercentage = (clickCoord - (boxStart+4))/142.0;

        System.out.println("Raw click coord is " + clickRawCoord);
        System.out.println("Sens from raw click coord is " + String.format("%.20f", getSensFromRawClickCoord(screen,clickRawCoord)));
        System.out.println("Expected sens is " + String.format("%.20f", boxPercentage));
        System.out.println("Expected sens is " + String.format("%.20f", (float)boxPercentage));
    }
}